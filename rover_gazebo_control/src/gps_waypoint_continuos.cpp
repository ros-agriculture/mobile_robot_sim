...
int countWaypointsInFile(std::string path_local) {
    ...
    if (fileCount.is_open()) {
        ...
        ROS_INFO("%.0f GPS waypoints are loaded", numWaypoints);
        fileCount.close();
    } else {
...
std::vector<std::pair<double, double>> getWaypoints(std::string path_local) {
    ...
    //Outputting vector
    ROS_INFO("GPS Waypoints have been set:");
...
int main(int argc, char **argv) {
    ...
    ros::Publisher pubWaypointNodeEnded = n.advertise<std_msgs::Bool>("/rover/waypoints_status", 100);
    n.setParam("/rover_gazebo_control/waypoints", "/waypoints.txt");
    ...
    ros::param::get("/rover_gazebo_control/waypoints", path_local);
    ...
    for (iter = waypointVect.begin(); iter < waypointVect.end(); iter++) {
        ...
        move_base_msgs::MoveBaseGoal goal = buildGoal(map_point, map_next,
                                                      final_point); //initiate a move_base_msg called goal
        ...
        if (ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED) {
            ...
            ROS_ERROR("Rover is unable to reach the waypoint");
