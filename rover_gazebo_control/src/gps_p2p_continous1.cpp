...
int countWaypointsInFile(std::string path_local) {
    path_abs = ros::package::getPath("rover_gazebo_control") + path_local;
...
int main(int argc, char **argv) {
    ...
    n.setParam("/rover_gazebo_control/coordinates_file", "/waypoints.txt");
    // Initiate publisher to send end of node message and publisher to say which node is publishing the proper vel commands
    ros::Publisher pubWaypointNodeEnded = n.advertise<std_msgs::Bool>("/rover_gazebo_control/waypoint_following_status",
                                                                      1000);
    ...
    controller_2_done.data = false;
    ros::param::get("/rover_gazebo_control/goalTolerance", goal_tolerance);
    ...
    ros::param::get("/rover_gazebo_control/coordinates_file", path_local);
    ...
    for (iter = waypointVect.begin(); iter < waypointVect.end(); iter++) {
        ...
        if (iter < (waypointVect.end() - 2)) // keep incrementing iter, we want to increment by two each time
        {
            iter++;
                   ...
                   == (waypointVect.end() -
                       2)) // this means controller 1 is on its last waypoint point but controller 2 has another to go
        {
            iter++;
            end_on_controller_1 = false;
        } else if (iter
                   == (waypointVect.end() -
                       1))  // this means that controller 1 is on THE last waypoint and should shutdown when it's done
        {
        ...
        } else //if on last waypoint, wait for it to acheive its goal
        {
            ...
            ROS_INFO("Rover has reached all of its goals!!!\n");
...
void waitToReachGoal(double map_x, double map_y, double goal_tolerance) {
    ...
    double last_distance_to_goal = sqrt(
            (map_x - x) * (map_x - x) + (map_y - y) * (map_y - y)), current_distance_to_goal;
