...
int main(int argc, char **argv) {
    ...
    double latitude_goal = 0, longitude_goal = 0;

    latitude_goal = 0.00005;
    longitude_goal = 0.00005;
...
//Transform UTM to odom frame
    ...
    while (notDone) {
        try {
            UTM_point.header.stamp = ros::Time::now();
            listener.waitForTransform("odom", "utm", ros::Time::now(), ros::Duration(0.2));
...
//Send goal to move_base
    ...
    cout << map_point.point.x << map_point.point.y << endl;
    ...
    if (ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
        ROS_INFO("Rover has reached it's goal");
    else
        ROS_INFO("Rover was unable to reach it's goal");
