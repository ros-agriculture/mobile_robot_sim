...
typedef rover_desc_pkg::TransmissionMsg myTransmissionMsg;
...
public:
    SubscribeAndPublish() {
        ...
        // read in parameters
        n.param<double>("vehicle_length", length, 1);
        n.param<double>("vehicle_width", width, 0.6);
    }
...
void applySteer(double array[3], double new_head, double length, double width) {
    ...
    // rho = length/(tan(abs(mainHead))); // bicycle model, mainHead is angle of a
    // wheel in middle of front wheel axis
    rho = length / (2 * tan(abs(mainHead)));  // point mass in middle of vehicle, mainHead
    // is angle the point will move at

    // turning right
    if (mainHead < 0.0) {
        if (array[0] < 1.5 && array[2] < 1.5 && array[0] > -1.5 && array[2] > -1.5) {
